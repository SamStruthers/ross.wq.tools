...
ex_plot <- function(df) {
  ...
  calibrated_data_df <- df %>%
    select(DT_round, value = mean_cal, correct_calibration)
  
  # Create plot with separate geom_line layers
  p <- ggplot() +
    geom_line(data = original_data, aes(x = DT_round, y = value), 
              color = "grey", linewidth = 0.3) +
    geom_line(data = calibrated_data_df, 
              aes(x = DT_round, y = value, color = correct_calibration), 
              linewidth = 0.5) +
...
plot_df <- calibrated_data$`2024`$`joei-pH`
ex_plot(plot_df)
```
