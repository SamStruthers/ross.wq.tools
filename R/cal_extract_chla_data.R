...
cal_extract_chla_data <- function(div) {
  ...
  div_tables <- div %>%
    html_elements("table") %>%
    discard(\(x) length(html_elements(x, "caption")) == 0)  %>%  # remove tables with no caption (empty placeholder tables)
    html_table() %>% # extract the table data
    set_names( # use the table caption to name the tables in the named_tables list
      {div %>%
